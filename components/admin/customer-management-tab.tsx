  ...
  useEffect(() => {
    if (searchQuery.trim() === "") {
      ...
      setFilteredCustomers(
        customers.filter(
          (c) =>
            c.full_name?.toLowerCase().includes(query) || c.email?.toLowerCase().includes(query) || c.phone?.toLowerCase().includes(query),
        ),
      )
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <div className="relative overflow-hidden rounded-xl border border-blue-100 bg-white p-4 shadow-sm">
          ...
          <p className="text-2xl font-bold text-blue-600 tabular-nums">{customers.filter((c) => !!c.phone).length}</p>
        </div>
