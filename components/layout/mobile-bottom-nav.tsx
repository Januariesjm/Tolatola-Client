...
import { Home, ShoppingCart, Grid3x3, ChevronRight, Bell } from "lucide-react"
    ...
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 px-3 pb-3 pt-1">
      <nav className="bg-white/95 backdrop-blur-xl border border-stone-200/90 shadow-[0_-4px_24px_rgba(0,0,0,0.08)] rounded-full h-16 flex items-center justify-around relative overflow-hidden">
        ...
        >
          ...
          {pathname === "/" && <div className="absolute -bottom-1 w-1.5 h-1.5 bg-[#1D61E7] rounded-full shadow-xs" />}
        </Link>
          ...
          <SheetContent side="bottom" className="h-[70vh] rounded-t-[2.5rem] border-none p-0 bg-white">
            ...
            <div className="overflow-y-auto h-full pb-20 p-6">
              <div className="flex flex-wrap justify-start gap-x-5 gap-y-5">
                {/* All Categories tile */}
                <Link href="/shop" onClick={() => setIsCategoriesOpen(false)} className="flex flex-col items-center gap-2 group">
                  ...
                  <span className="text-[11px] font-extrabold text-primary text-center leading-tight max-w-[72px]">
                    {t("category.all")}
                  </span>
                </Link>
                  ...
                  .map((cat) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-[11px] font-bold text-stone-700 text-center leading-tight max-w-[72px] line-clamp-2">
        ...
        >
          ...
          {pathname === "/messages" && <div className="absolute -bottom-1 w-1.5 h-1.5 bg-[#1D61E7] rounded-full shadow-xs" />}
        </Link>
        ...
        >
          <div className="relative">
            <ShoppingCart
              className={cn("h-5 w-5 transition-transform", pathname === "/cart" ? "scale-110 stroke-[2.4]" : "stroke-[1.8]")}
            />
            {cartCount > 0 && (
              <span className="absolute -top-1.5 -right-2 bg-[#E53E3E] text-white text-[8px] font-extrabold h-4 w-4 rounded-full flex items-center justify-center ring-2 ring-white shadow-xs">
          ...
          {pathname === "/cart" && <div className="absolute -bottom-1 w-1.5 h-1.5 bg-[#1D61E7] rounded-full shadow-xs" />}
        </Link>
      </nav>
    </div>
  )
