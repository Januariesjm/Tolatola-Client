...
import { getInitials, useSiteHeaderState } from "@/hooks/use-site-header-state"
...
export default function SiteHeader({ user, profile, kycStatus }: SiteHeaderProps) {
  ...
  const { authUser, authProfile, authKyc, mobileMenuOpen, setMobileMenuOpen, scrolled, cartCount, handleLogout, isVerified, isHome } =
    useSiteHeaderState({ user, profile, kycStatus })
          ...
          <div className="flex lg:hidden items-center justify-between w-full gap-1 sm:gap-1.5 md:gap-2 h-full px-0.5">
              ...
              <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                <SheetTrigger asChild>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-8.5 w-8.5 rounded-xl bg-stone-50 dark:bg-stone-900 hover:bg-stone-100 dark:hover:bg-stone-800 text-stone-900 dark:text-stone-100 p-0"
                  >
                ...
                <SheetContent
                  side="left"
                  hideCloseButton
                  className="w-full sm:w-[400px] border-none p-0 bg-white dark:bg-stone-950 text-stone-900 dark:text-stone-100 z-[200]"
                >
                    ...
                    <SheetHeader className="p-6 text-left bg-stone-950 dark:bg-stone-900 text-white relative">
                      ...
                      <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-primary dark:text-blue-400">
                            {t("nav.navigation")}
                          </p>
                        </div>
                      </div>
                          ...
                          <Link key={item.href} href={item.href} onClick={() => setMobileMenuOpen(false)}>
                            <div className="flex items-center justify-between p-5 rounded-2xl bg-stone-50 dark:bg-stone-900 hover:bg-stone-900 dark:hover:bg-stone-800 hover:text-white transition-all group">
                              <div className="flex items-center gap-4">
                                ...
                                <span className="text-lg font-bold tracking-tight text-stone-900 dark:text-stone-100 group-hover:text-white">
                                  {item.label}
                                </span>
                              </div>
            ...
            {authUser ? (
              <DropdownMenu>
                ...
                <DropdownMenuContent
                  className="w-64 mt-4 p-2 rounded-[2rem] bg-white dark:bg-stone-900 border-stone-100 dark:border-stone-800 shadow-2xl z-[150]"
                  align="end"
                >
                  ...
                  <DropdownMenuItem
                    ...
                    className="rounded-xl h-12 cursor-pointer text-destructive focus:bg-destructive/5 font-black text-xs uppercase tracking-widest px-4"
                  >
          ...
          <div className="hidden lg:flex items-center justify-between w-full gap-4 h-full">
            ...
            <nav className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                {authUser ? (
                  <div className="flex items-center gap-3">
                    <NotificationPopover userType={authProfile?.user_type ?? undefined} />
                  ...
                  <div className="flex items-center gap-3">
                    <Link href="/auth/login">
                      <Button
                        variant="ghost"
                        className="font-black text-xs uppercase tracking-[0.2em] text-stone-500 dark:text-stone-400 hover:text-primary dark:hover:text-blue-400 bg-transparent"
                      >
                        {t("nav.login")}
                      </Button>
                    </Link>
